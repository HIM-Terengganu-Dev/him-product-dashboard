...
export default function ProductGMVRecordsPage() {
    ...
    const fetchRecords = useCallback(async () => {
        ...
        setError(null);
        try {
            const response = await fetch('/api/tiktok/product-gmv/dates');
    ...
    const fetchLogs = useCallback(async (date?: string) => {
        ...
        try {
            const url = date 
                ? `/api/tiktok/product-gmv/logs?date=${encodeURIComponent(date)}&limit=50`
                : `/api/tiktok/product-gmv/logs?limit=100`;
    ...
    const handleDelete = async (date: string) => {
        ...
        try {
            ...
            const response = await fetch(`/api/tiktok/product-gmv/delete?date=${dateParam}${userEmailParam}`, {
                method: 'DELETE',
            });
    ...
    const getOperationColor = (operationType: string) => {
        ...
        if (type === 'upload' || type === 'manual_entry') {
                    ...
                    <div className="flex items-center justify-between">
                        <div>
                            <h1 className="text-2xl font-bold bg-gradient-to-r from-pink-600 to-red-600 bg-clip-text text-transparent">
                                Product GMV Records Management
                            </h1>
                            <p className="text-sm text-gray-600 mt-1">Manage and view all uploaded Product GMV data by date</p>
                        </div>
                        <div className="flex items-center gap-3">
                            <Link
                                href="/sales-portal/tiktok/product-gmv/upload"
                                className="px-4 py-2 text-sm font-medium text-white bg-gradient-to-r from-pink-600 to-red-600 rounded-lg hover:from-pink-700 hover:to-red-700 transition-colors shadow-md"
                            >
                                📤 Upload Data
                            </Link>
                            <Link
                    ...
                    <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
                        ...
                        {records.length === 0 ? (
                            <div className="p-12 text-center">
                                ...
                                <p className="text-gray-400 text-sm mb-6">Upload your first Product GMV data to get started</p>
                                <Link
                                    href="/sales-portal/tiktok/product-gmv/upload"
                                    className="inline-block px-6 py-3 text-sm font-medium text-white bg-gradient-to-r from-pink-600 to-red-600 rounded-lg hover:from-pink-700 hover:to-red-700 transition-colors shadow-md"
                                >
                                    Upload Data
                                </Link>
                            </div>
                                            ...
                                            <tr key={record.report_date} className="hover:bg-gray-50">
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                    <div className="flex items-center justify-end gap-2">
                                                        <Link
                                                            href={`/sales-portal/tiktok/product-gmv/upload?date=${record.report_date}`}
                                                ...
                                                {logs.map((log) => (
                                                    <tr key={log.id} className="hover:bg-gray-50">
                                                        ...
                                                        <td className="px-4 py-3 whitespace-nowrap">
                                                            <span className={`px-2 py-1 rounded-full text-xs font-semibold ${getOperationColor(log.operation_type)}`}>
