    ...
    try {
      ...
      const formattedData = validRows.map((row) => {
        ...
        return {
          ...
          campaign_group: campaignGroup || row.campaign_name.trim(), // Fallback to campaign_name if no group detected
          cost: parseFloat(row.cost) || 0,
          net_cost: parseFloat(row.net_cost) || parseFloat(row.cost) || 0,
          live_views: parseInt(row.live_views) || 0,
      ...
      const response = await fetch('/api/tiktok/live-gmv/manual-entry', {
        method: 'POST',
        headers: {
        ...
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-600 mt-1">Enter Live GMV campaign data manually</p>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-6">
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Live Views</th>
                  ...
                  <tr key={row.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">
                      <input
                        ...
                        placeholder="Campaign Name or [Group] Name"
                        className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                        disabled={submitting}
                      />
                    </td>
                    <td className="px-4 py-3">
                      <input
                        ...
                        placeholder="0.00"
                        className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg text-right focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                        disabled={submitting}
                      />
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      <input
                        type="number"
                        value={row.live_views}
                        onChange={(e) => updateRow(row.id, 'live_views', e.target.value)}
                        placeholder="0"
                        className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg text-right focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                        disabled={submitting}
                      />
        ...
        <div className="flex justify-end gap-3">
          ...
          <button
            ...
            className="px-6 py-3 text-sm font-medium text-white bg-gradient-to-r from-indigo-600 to-purple-600 rounded-lg hover:from-indigo-700 hover:to-purple-700 transition-colors shadow-md disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {submitting ? (
