...
export default async function handler(
  ...
  try {
    ...
    const campaigns: CampaignPerformance[] = result.rows.map(row => ({
      ...
      live_views: parseInt(row.live_views) || 0, // Not applicable for Product GMV but kept for type compatibility
    ...
    console.error('Error fetching Product GMV campaign performance:', error);
