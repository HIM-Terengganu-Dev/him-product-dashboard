  ...
  try {
    ...
    const groups: GroupPerformance[] = result.rows.map(row => {
      ...
      return {
        ...
        total_live_views: parseInt(row.total_live_views) || 0, // Not applicable for Product GMV but kept for type compatibility
        roas: roas,
    ...
    console.error('Error fetching Product GMV group performance:', error);
